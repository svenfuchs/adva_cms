module Menus
	class Base < Tags::Node
	  attr_accessor :id, :url, :branch, :options, :active
		...
		def initialize(id, options = {})
		  @id = id
		  [:text, :content, :url, :branch, :level].each { |key| instance_variable_set(:"@#{key}", options.delete(key)) }
		  super(options)
	  end
    ...
    def activate(path)
      ...
      @sections.each { |section| section.active = true if section.url.starts_with(path) }
    end
	  ...
	  def content
	    @content ||= begin
	      ...
	      tag.render + Tags::Ul.new(:id => 'sections_menu').render do |html|
