...
describe "Message views:" do
  ...
  before :each do
    ...
    template.stub!(:render).with hash_including(:partial => 'message_nav')
  ...
  describe "_inspect" do
    ...
    it "has message sender name in a paragraph" do
      act!
      response.should have_tag('p.message_sender', /from: #{@message.sender.name}/)
    end

    it "has message body in a paragraph" do
      pending 'FIXME'
      act!
      response.should have_tag('div.message_body', /#{@message.body}/)
    end
  end
