...
class ThemesHelperTest < ActionView::TestCase
  ...
  test 'theme_image_tag' do
    ...
    tag.should == '<img alt="Rails" src="/themes/a-theme/images/rails.png" />'
  end

  test 'theme_javascript_include_tag' do
    tag = theme_javascript_include_tag('a-theme', 'foo')
    tag.should == '<script src="/themes/a-theme/javascripts/foo.js" type="text/javascript"></script>'
  end

  test 'theme_javascript_include_tag with :all' do
    tag = theme_javascript_include_tag('a-theme', :all)
    tag.should == '<script src="/themes/a-theme/javascripts/effects.js" type="text/javascript"></script>'
  end
  
  test 'theme_javascript_include_tag with caching' do
    tag = theme_javascript_include_tag('a-theme', 'effects', :cache => true)
    tag.should == '<script src="/themes/a-theme/javascripts/all.js" type="text/javascript"></script>'
    "#{@theme.path}/javascripts/all.js".should be_file
  end

  test 'theme_stylesheet_link_tag' do
    tag = theme_stylesheet_link_tag('a-theme', 'foo')
    tag.should == '<link href="/themes/a-theme/stylesheets/foo.css" media="screen" rel="stylesheet" type="text/css" />'
  end

  test 'theme_stylesheet_link_tag with :all' do
    tag = theme_stylesheet_link_tag('a-theme', :all)
    tag.should == '<link href="/themes/a-theme/stylesheets/styles.css" media="screen" rel="stylesheet" type="text/css" />'
  end
  
  test 'theme_stylesheet_link_tag with caching' do
    tag = theme_stylesheet_link_tag('a-theme', 'styles', :cache => true)
    tag.should == '<link href="/themes/a-theme/stylesheets/all.css" media="screen" rel="stylesheet" type="text/css" />'
