...
class TestSafemodeEval < Test::Unit::TestCase
  ...
  def test_some_stuff_that_should_work
    ['"test".upcase', '10.succ', '10.times{}', '[1,2,3].each{|a| a + 1}', 'true ? 1 : 0', 'a = 1'].each do |code|
      assert_nothing_raised{ safebox_eval(code) }
    end
  end
  ...
  def test_should_turn_locals_to_jails
    assert_raise(Safemode::NoMethodError){ safebox_eval("article.system", {}, @locals) }
  end
  
  def test_should_allow_method_access_on_assigns
    assert_nothing_raised{ safebox_eval("@article.title", @assigns) }
  end
  
  def test_should_allow_method_access_on_locals
    assert_nothing_raised{ safebox_eval("article.title", {}, @locals) }
  end
  
  def test_should_not_raise_on_if_using_return_values
    assert_nothing_raised{ safebox_eval("if @article.is_article? then 1 end", @assigns) }
  end
  
  def test_should_work_with_if_using_return_values
    assert_equal safebox_eval("if @article.is_article? then 1 end", @assigns), 1
  end
  
  def test__FILE__should_not_render_filename
    assert_equal '(string)', safebox_eval("__FILE__")
  end
