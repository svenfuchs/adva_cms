...
describe Admin::ArticlesController do
  ...
  before :each do
    ...
    @author = stub_user 
    @article = Article.create :title => 'article title', :body => 'article body', :section => @blog, :author => @author
    ...
    @controller.stub!(:update_all)
  end

  describe "after save" do
    it "does not ping a blog service if article is not published" do
      Article.should_receive(:find).and_return(@article)
      @article.should_receive(:published?)
      @observer.should_not_receive(:ping_service)
      request_to :put, "/admin/sites/#{@site.id}/sections/#{@blog.id}/articles/#{@article.id}", :draft => 1 #{:"published_at(1i)" => "2008", :"published_at(2i)"=>"9", :"published_at(3i)"=>"29"}
    end

#    it "does a pong_get_ping ping if article is published and pom_get_ping is set as service" do
#      ArticlePingObserver::SERVICES << { :url => "http://pom_get_ping.com/", :type => :pom_get }
#      Article.should_receive(:find).and_return(@article)
#      @observer.should_receive(:pom_get_ping).with("http://pom_get_ping.com/", @article, nil)
